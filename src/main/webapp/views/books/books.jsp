...
<body class="hold-transition sidebar-mini layout-fixed">
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<div class="card">
				<div class="card-header">
					...
					<form class="form-inline ml-3" style="float: right;" id="form-search" action="/books?action=SEARCH"
						method="post">
						<div class="input-group input-group-sm">
							<input class="form-control form-control-navbar" type="hidden" placeholder="Search"
								aria-label="Search" name="action" value="LIST">
				...
				<div class="card-body">
                  ...
                  %>
							<tr>
								...
								<td><img src="/img/<%=item.getImg_url() %>" style="width:50px;height:50px"> </td>
								<td><%=item.getTotalBook() %></td>
								<td style="display: flex;">
									<a class="s" style="width: 20px;height: 20px"
										href="/books?action=EDIT&id=<%=item.getId()%>">

										<i class="fas fa-pen"></i>
									</a>|

									<btn id="btn-delete" class="btn-delete" onclick="deleleByID(<%=item.getId()%>)">
										<input type="hidden" value="<%=item.getId()%>" class="id-delete">
		...
		<script type="text/javascript">
			...
			// 	e.preventDefault();
			// 	debugger;
			// 	var id = $("#btn-delete .id-delete").val();
			// 	swal({
			...
			// 		if (isConfirm.value) {
			// 			window.location.href = "/books?action=DELETE&id=" < %= % > ;
